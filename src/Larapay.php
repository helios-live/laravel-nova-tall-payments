...
class Larapay {
    ...
    public function __construct() {
        ...
        $this->gateways = [
            'admin' => [
                ...
                'filter' => function(){ return Gate::check('pay-manual', new Invoice()); },
            ],
            'stripe' => [
                'name' => 'Stripe',
                ...
                'filter' => function(){ return !in_array('stripe', config('larapay.disable_payment_methods')); },
            ],
            'paypal' => [
                'name' => 'Paypal',
                ...
                'filter' => function(){ return !in_array('paypal', config('larapay.disable_payment_methods')); },
            ],
            'bitcoin' => [
                'name' => 'Bitcoin',
                ...
                'filter' => function(){ return !in_array('bitcoin', config('larapay.disable_payment_methods')); },
            ]
        ];
    }
    ...
    public function dashboard() {
        ...
        $subs = collect();
        if ( $team ) {
            $subs = $team->subscriptions()->
                    orderBy('id', 'desc')->
                    with(['price.product', 'latestInvoice'])->
                    get();
        }
        $data = [
            'subscriptions' => $subs,
    ...
    public function processResult($result, $gateway, $invoice)
    {
        ...
        if ($result->status == "succeeded") {
            ...
            $messageBag->add('message', $result->message);
